...
- リポジトリの**実体ディレクトリ `~/dev/nozei-schedule` は「main 統合＋デプロイ専用」**。
...
git fetch origin
git worktree add ../nozei-schedule-<topic> -b feat/<topic> origin/main
# 起点ズレ確認（0 であること）
git -C ../nozei-schedule-<topic> rev-list --count origin/main..HEAD

# 一覧 / 後片付け
git worktree list
git worktree remove ../nozei-schedule-<topic>   # マージ後に撤去
```

- 命名規則: `feat/<topic>` `fix/<topic>` `chore/<topic>`。worktree dir は `../nozei-schedule-<topic>`。
- **同じファイルを2つの worktree で同時編集しない**（役割境界を守る）。
- worktree は本体 repo の `node_modules` を symlink すると検証が速い（`ln -s ../nozei-schedule/node_modules node_modules`）。
...
- **`main にマージ ＝ 本番(Production)自動デプロイ`** に一本化（Cloudflare Pages の Git 連携）。
- **手動デプロイは原則禁止**（本番状態の二重化を防ぐ）。緊急時のみ番人が実施し、必ず記録。
- **PR ごとに自動でプレビュー URL が発行**される。→ **機能別の動作確認はプレビュー URL で**。本番は main だけ。
- 環境変数は Cloudflare Pages ダッシュボードが正本（`.env.local` はローカル dev 用、リポに出さない）。
